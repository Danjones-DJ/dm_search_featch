...
# Define the sim_degrees function for modal search
sim_degrees <- function(word, df = degree_data) {
  ...
  div(id = "courseModal", class = "custom-modal",
      div(class = "modal-content-custom",
          ...
          div(class = "modal-two-columns",
              ...
              # Search section (second column)
              div(class = "search-section",
                  div(class = "features-title", "Search Similar Courses"),
                  div(class = "search-controls",
                      textInput("plot_search_word", "Enter keyword:", value = "", 
                                placeholder = "e.g. math, physics, computer"),
                      actionButton("search_submit", "Search", class = "btn-primary")
                  ),
                  div(class = "search-results",
                      div(style = "display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;",
                          div(class = "features-title", style = "margin: 0;", "Search Results"),
                          actionButton("fullscreen_plot", "⛶ Fullscreen", class = "btn btn-sm", 
                                       style = "background: rgba(75,144,226,0.2); border: 1px solid #4b90e2; color: #4b90e2; padding: 5px 10px; font-size: 11px;")
                      ),
                      plotOutput("modal_chart_plot", height = "350px", width = "100%")
                  )
              )
          )
      )
  ...
  div(id = "fullscreenModal", class = "custom-modal",
      div(style = "background: linear-gradient(135deg, #191414 0%, #1a1a1a 100%); border-radius: 16px; max-width: 95vw; max-height: 95vh; margin: auto; padding: 20px; color: white; position: relative; overflow: hidden;",
          ...
          div(style = "margin-bottom: 20px;",
              h3(style = "color: #4b90e2; margin-bottom: 10px;", "Course Search Results")
          ),
          plotOutput("fullscreen_chart_plot", height = "70vh", width = "100%")
      )
  ),
...
server <- function(input, output, session) {
  ...
  # Modal search state - using reactive values to avoid multiple output assignments
  ...
  output$modal_chart_plot <- renderPlot({
    modal_search_state$plot
  })
  
  # Fullscreen plot outputs (mirror the modal plot)
  output$fullscreen_chart_plot <- renderPlot({
    modal_search_state$plot
  })
  ...
  observeEvent(input$reset_modal_search, {
    ...
    updateTextInput(session, "modal_search_input", value = "")
  ...
  observeEvent(input$search_submit, {
    ...
    if(is.null(search_word) || search_word == "" || trimws(search_word) == "") {
      ...
      modal_search_state$plot <- ggplot() + 
        annotate("text", x = 0.5, y = 0.5, 
      ...
      # Call the sim_degrees function (always sorted by salary)
      tryCatch({
        plot_result <- sim_degrees(search_word, degree_data)
        modal_search_state$plot <- plot_result
